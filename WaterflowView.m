...
@synthesize notifycationName=_notifycationName;

-(void)notificationName:(NSString *)name{
    self.notifycationName = name;
}
...
- (void)pageScroll
{
    ...
    for (int i = 0 ; i< numberOfColumns; i++)
    {
        ...
        {
            int rowToDisplay = 0;
            if ([[self.cellHeight objectAtIndex:i] count]>0) {//一列有数据才处理
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:self.reuseIdentifier
                                                        object:self
